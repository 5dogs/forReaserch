"""
固定税額（従量税）と従価税率の比較シミュレーション（税率除外版）
...
print("="*60)
print("固定税額 vs 従価税率 シミュレーション（税率除外版）")
print("="*60)
...
annual_file = os.path.join(project_root, 'demand_regression_data_annual.csv')
...
plt.tight_layout()
output_file = os.path.join(figures_dir, '09_fixed_vs_advalorem_tax_comparison.png')
...
result_file = os.path.join(output_dir, '06_fixed_vs_advalorem_simulation.csv')
