...
df_analysis = pd.read_csv(data_file)

# 消費者余剰の結果
cs_file = os.path.join(project_root, 'analysis', 'results', 'excl_tax', '02_consumer_surplus_results.csv')
...
# ============================================================================
# Graph 1: 需要関数の推定結果（係数の可視化）- 税率弾力性（γ）を除外
# ============================================================================
print("\nCreating Graph 1: Demand Function Coefficients (Tax Rate Excluded)...")
fig, ax = plt.subplots(figsize=(12, 6))

# 弾力性係数（税率弾力性を除外）
coeff_names = ['Income\nElasticity (α)', 'Price\nElasticity (β)']
coeff_values = [coefficients['alpha'], coefficients['beta']]
...
ax.set_title('Estimated Demand Function Coefficients (Tax Rate Excluded)', fontweight='bold')
ax.grid(True, alpha=0.3, linestyle='--', axis='y')
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, '01_demand_function_coefficients.png'), dpi=300, bbox_inches='tight')
print(f"  Saved: {os.path.join(figures_dir, '01_demand_function_coefficients.png')}")
plt.close()
...
for event_date_str, event_info in policy_events.items():
    ...
    if not pd.isna(event_date) and event_date >= df_cs['Date'].min():
        ...
        # 2009年のラベルは横書きにしてグラフ下部、縦線の右隣に配置（重なりを避ける）
        if event_date_str == '2009':
            y_pos = ax.get_ylim()[0] * 0.95  # グラフ下部に配置
            # X座標を少し右にずらして縦線の右隣に配置
            x_offset = pd.Timedelta(days=180)  # 約6ヶ月分右にずらす
            ax.text(event_date + x_offset, y_pos, event_info['label'],
                   rotation=0, verticalalignment='bottom', horizontalalignment='left',
                   fontsize=12, color=event_info['color'])
        else:
            y_pos = ax.get_ylim()[1] * 0.95  # 通常の位置（縦書き）
            ax.text(event_date, y_pos, event_info['label'],
...
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, '02_consumer_surplus_increase.png'), dpi=300, bbox_inches='tight')
print(f"  Saved: {os.path.join(figures_dir, '02_consumer_surplus_increase.png')}")
plt.close()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, '03_cumulative_consumer_surplus.png'), dpi=300, bbox_inches='tight')
print(f"  Saved: {os.path.join(figures_dir, '03_cumulative_consumer_surplus.png')}")
plt.close()
