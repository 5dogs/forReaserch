# フォルダ・ファイル整理内容（統合版）

> **注意**: このドキュメントは`整理完了報告.md`と統合し、最新の状態を反映しています（2025年11月26日更新）

## 📁 整理日時
2025年11月26日

## 実施した整理内容

### 1. データ準備スクリプトの整理

**移動先**: `scripts/data_preparation/`

以下のスクリプトを`scripts/data_preparation/`フォルダに移動しました：

- `00_prepare_log_transformed_data.py` - 対数変換データ準備（四半期データ版）
- `02_complete_consumption_data.py` - 消費量データ補完
- `03_fix_units.py` - 単位統一
- `04_process_cpi_data.py` - CPIデータ処理
- `05_add_cpi_and_relative_price.py` - CPIと相対価格の追加
- `06_aggregate_to_annual_data.py` - 年次データ集約
- `07_prepare_annual_log_transformed_data.py` - 年次データ対数変換（最新版）
- `add_gdp_data.py` - GDPデータ追加
- `add_price_data_1990.py` - 価格データ追加
- `add_tax_rate_data.py` - 税率データ追加

**理由**: データ準備関連のスクリプトを一箇所にまとめることで、管理しやすくしました。

### 2. 不要・重複ファイルの整理

**削除したファイル**:

- `analysis/01_estimate_demand_function.py` (差分モデル版)
- `analysis/01_estimate_demand_function_level_model.py` (四半期レベルモデル版)
- `analysis/results/01_coefficients.json`
- `analysis/results/01_coefficients_level_model.json`
- `analysis/results/01_demand_function_coefficients.csv`
- `analysis/results/01_demand_function_coefficients_level_model.csv`
- `analysis/results/01_analysis_data.csv`
- `analysis/results/01_analysis_data_level_model.csv`
- `analysis/demand_regression_data_log_transformed.csv` (四半期版)
- `memo.text` (古いメモ、`project/学士論文メモ.text`が正式版)

**理由**: 現在使用していないスクリプトや中間ファイルを削除し、最新版のみを保持しました。

### 3. visualizationフォルダの重複整理

**削除したファイル・フォルダ**:

- `visualization/create_raw_data_visualizations.py` - 重複（`01_create_raw_data_visualizations.py`が正式版）
- `visualization/visualization/` - 重複フォルダ

**理由**: 重複ファイルを削除し、`01_create_raw_data_visualizations.py`を正式版として統一しました。

## 📂 整理後のフォルダ構成

```
forReaserch/
├── analysis/                    # 分析スクリプトと結果
│   ├── 01_estimate_demand_function_annual_level_model.py  # 最新版: 年次データ・レベルモデル推定
│   ├── 02_calculate_consumer_surplus.py
│   ├── 03_visualize_results.py
│   ├── demand_regression_data_annual_log_transformed.csv
│   ├── figures/                # グラフ
│   └── results/                # 分析結果（最新版のみ）
│       ├── 01_coefficients_annual_level_model.json
│       ├── 01_analysis_data_annual_level_model.csv
│       ├── 01_demand_function_coefficients_annual_level_model.csv
│       └── 02_consumer_surplus_results.csv
├── scripts/                    # データ準備スクリプト
│   └── data_preparation/
│       ├── 00_prepare_log_transformed_data.py
│       ├── 02_complete_consumption_data.py
│       ├── 03_fix_units.py
│       ├── 04_process_cpi_data.py
│       ├── 05_add_cpi_and_relative_price.py
│       ├── 06_aggregate_to_annual_data.py
│       ├── 07_prepare_annual_log_transformed_data.py
│       ├── add_gdp_data.py
│       ├── add_price_data_1990.py
│       ├── add_tax_rate_data.py
│       └── README.md
├── visualization/              # 可視化スクリプト
│   ├── 01_create_raw_data_visualizations.py
│   └── figures/
├── data/                       # データファイル
│   ├── -2025ガソリン関連税四半期ごと/
│   ├── -2025消費者物価指数/
│   ├── 1990-2025_ガソリン小売価格四半期ごと/
│   ├── 1994-2025_GDP四半期ごと/
│   └── 2007-2024ガソリン販売量/
├── project/                    # プロジェクトメモ
│   ├── 前年度提出論文_コピペ.text
│   ├── 学士論文メモ.text
│   └── 学士論文目次.text
├── 消費者余剰測定方法まとめ/   # 測定方法ドキュメント
├── demand_regression_data_raw.csv      # 元データ（四半期）
├── demand_regression_data_annual.csv   # 年次データ
├── README.md
├── スクリプト・結果.md
├── プラン.md
├── 整理内容.md (このファイル)
├── 整理完了報告.md
├── GDPデータ取得方法.md
└── 消費者物価指数データ取得方法.md
```

## 🔄 スクリプトのパス修正

### 修正したスクリプト

1. **`scripts/data_preparation/07_prepare_annual_log_transformed_data.py`**
   - ルートディレクトリの`demand_regression_data_annual.csv`を読み込む
   - 出力先を`analysis/demand_regression_data_annual_log_transformed.csv`に設定

2. **`analysis/01_estimate_demand_function_annual_level_model.py`**
   - `analysis/demand_regression_data_annual_log_transformed.csv`を読み込む（年次データ版）

## 現行の分析フロー

1. **データ準備** (`scripts/data_preparation/`)
   - 各データソースからデータを統合
   - 単位統一、欠損値処理
   - CPIと相対価格の計算
   - 年次データへの集約（`06_aggregate_to_annual_data.py`）
   - 年次データの対数変換（`07_prepare_annual_log_transformed_data.py`）

2. **分析** (`analysis/`)
   - `01_estimate_demand_function_annual_level_model.py`: 年次データでレベルモデル推定（R²=93.9%）
   - `02_calculate_consumer_surplus.py`: 消費者余剰計算
   - `03_visualize_results.py`: 結果可視化

3. **結果** (`analysis/results/`)
   - 年次データ版の推定結果のみ保持

## ✅ 整理の効果

1. **明確な構造**: データ準備、分析、可視化のスクリプトが明確に分離
2. **管理しやすさ**: 関連スクリプトが一箇所にまとまり、管理が容易
3. **ルートディレクトリの整理**: 主要ファイルのみがルートに残り、見通しが良くなった
4. **再現性の向上**: スクリプトの役割が明確になり、再現性が向上
5. **最新版のみ保持**: 古いバージョンのスクリプトや結果ファイルを削除し、混乱を防止

## 📝 今後の注意点

- 新しいデータ準備スクリプトは`scripts/data_preparation/`に配置
- 分析スクリプトは`analysis/`に配置
- 可視化スクリプトは`visualization/`に配置
- 不要になったファイルは削除するか、`archive/`に移動（現在は`archive/`フォルダは削除済み）
- 最新の分析結果は年次データ版（R²=93.9%）です

## 注意事項

- 古いバージョンのスクリプトや結果ファイルは削除済み
- 現行のコードのみが残っています
- 最新の分析結果は年次データ版（R²=93.9%）です
