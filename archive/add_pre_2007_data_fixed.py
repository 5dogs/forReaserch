...
print("2007年以前のデータを追加します（修正版）...\n")

# 1. 既存のdemand_regression_data_raw.csvを読み込む（2007年以降のデータのみ）
target_file = 'demand_regression_data_raw.csv'
df_main = pd.read_csv(target_file)
df_main['Year'] = df_main['Year'].astype(str)

# 2007年以降のデータのみを保持
df_main = df_main[df_main['Year'].str[:4].astype(int) >= 2007].copy()
print(f"既存データ（2007年以降）: {len(df_main)}行")
...
for idx, row in df_sales_raw.iterrows():
    # 行をリストに変換（NaNも含める、文字列に変換）- デバッグスクリプトと同じ方法
    row_list = [str(v) if pd.notna(v) else '' for v in row]
    if len(row_list) < 10:
        continue
    
    # 最後の列にC.Y.が含まれているか確認
    last_col = row_list[-1] if row_list else ""
    ...
    heisei = None
    if len(row_list) > 3:
        col2 = row_list[2] if row_list[2] != '' else ''
        col3 = row_list[3] if row_list[3] != '' else ''
    ...
    if heisei and heisei in heisei_to_year:
        ...
        # ガソリン列は列7（インデックス7）
        if len(row_list) > 7:
            gasoline_str = row_list[7].replace(',', '').replace('"', '').strip()
            # 数値チェック
            try:
                ...
                # デバッグ用
                if idx < 15:
                    print(f"  [DEBUG] 行{idx+1}: gasoline_str='{gasoline_str}', error={e}")
                pass
...
# 重複を除去（Year列で）
df_output = df_output.drop_duplicates(subset=['Year'], keep='first')
